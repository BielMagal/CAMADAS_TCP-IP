...
if ($protocolo == UDP) {
    ...
    deletar(SEGMENTO);
    echo "TRANSPORTE - Enviando requisição para APLICACAO..." . PHP_EOL;
    ...
    deletar(SEGMENTO);
    if(strcmp (CONEXAO, substr($resposta, 0, -2)) == 0){
      deletar(MENSAGEM);
      echo "TRANSPORTE - Solicitaçao de conexao recebida!..." . PHP_EOL;
      echo "TRANSPORTE - Enviando confirmacao de conexao..." . PHP_EOL;
      ...
      $conteudo = fwrite($arquivo, $segmento);
      fclose($arquivo);
    }else if((strcmp (ACK, substr($resposta, 0, -2)) == 0)){
      deletar(MENSAGEM);
      $segmento = criaSegmentoTCP(0, OK) . PHP_EOL . PHP_EOL . FIMMSG . PHP_EOL;
      $seg_name = SEGMENTO . '0';
      // Escreve no segmento para mandar
